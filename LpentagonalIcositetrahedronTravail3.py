...
nom_polyedre= "LpentagonalIcositetrahedron"
    ...
    for ixs in les_faces:
        ...
        if abs(largeur[nb_faces]) > 0:
            if abs(marge[nb_faces]) > 0:
                    ...
                    offset2D(largeur[nb_faces],"arc").twistExtrude(epaisseur[nb_faces],40).\
                    add(lines).toPending().offset2D(largeur[nb_faces]-marge[nb_faces],"arc").\
                    twistExtrude(epaisseur[nb_faces],35,combine="cut")
...
exporters.export(Polyedre(),"./stl/" + nom_polyedre + "Travail3.stl",tolerance=0.2)
