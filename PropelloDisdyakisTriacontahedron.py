...
nom_polyedre= "PropelloDisdyakisTriacontahedron"
...
sommets = [(a * factor, b * factor, c * factor) for (a,b,c) in sommets]
    ...
    for ixs in les_faces:
        ...
        if extension[nb_faces] > 0:
            if trottoir[nb_faces] > 0:
                    ...
                    offset2D(extension[nb_faces],"intersection").extrude(tranche[nb_faces]).faces().end().workplane().\
                    add(une_face).wires().toPending().offset2D(extension[nb_faces]-trottoir[nb_faces],"arc").extrude(tranche[nb_faces],combine="cut")
            else:   
                    dessus = Workplane(une_face).faces().workplane().add(une_face).wires().toPending().\
                    offset2D(extension[nb_faces],"intersection").extrude(tranche[nb_faces])
