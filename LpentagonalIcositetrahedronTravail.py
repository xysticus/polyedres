    ...
    for ixs in les_faces:
        ...
        if largeur[nb_faces] > 0:
            if marge[nb_faces] > 0:
                    ...
                    add(lines).toPending().offset2D(largeur[nb_faces]-marge[nb_faces],"arc").extrude(epaisseur[nb_faces],combine="cut")
            else:   
...
exporters.export(Polyedre(),"./stl/" + nom_polyedre + "Travail.stl",tolerance=0.2)
